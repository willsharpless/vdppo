...
from rraa_rl.rl.utils.utils import get_BuRd
from PIL import Image
import imageio
import wandb 
from time import time 

from rraa_rl.env.reach_avoid.safety_gym_RR import PointReachReach, PointReach1, PointReach2
from rraa_rl.env.reach_avoid.safety_gym_RAA import PointReachAvoid, PointAvoidOnly
from rraa_rl.env.reach_avoid.safety_gym_RRAA import PointRRAA
from rraa_rl.env.reach_avoid.half_cheetah_RAA import HalfCheetahReachAvoid, HalfCheetahAvoidOnly
from rraa_rl.env.reach_avoid.half_cheetah_RR import HalfCheetahReachReach, HalfCheetahReach1, HalfCheetahReach2
from rraa_rl.env.reach_avoid.humanoid_RR import HumanoidReachReach, HUMANOID_TARGET_RIGHT, HUMANOID_TARGET_LEFT, HUMANOID_TARGET_RADIUS
from rraa_rl.env.reach_avoid.humanoid_RAA import HumanoidReachAvoid, HUMANOID_RAA_TARGET, HUMANOID_RAA_TARGET_RADIUS, HUMANOID_RAA_BOX_RADIUS, HUMANOID_RAA_FLOOR_HEIGHT
from jax import jit
...
def plot_contour_RRAA(multi_info, epoch, config, policy_decision_sample=None):
    ...
    if 'Hopper' in config['EXP_NAME'] and 'ReachReach' in config["EXP_NAME"]: 
        ...
        def draw_point_raa(info, title, ax):
            reach_idx = info.get('reach_index')
            avoid_idx = info.get('avoid_index')
            full_len = info['x'].shape[0]
            ...
            for step_n, i in enumerate(indices):
                ...
                reach_val = is_reach_np(np.array([info['x'][i].item(), info['y'][i].item()]))
        ...
        # Draw Reach Avoid and Avoid Only 
        draw_point_raa(info, "Reach Avoid", axes[0])
...
def plot_video_contour_RRAA(multi_info, epoch, config, save_video=False, prefix="", log_wandb=True):
    ...
    if 'Hopper' in config['EXP_NAME'] and 'Avoid' in config['EXP_NAME']: 
        ...
        def draw_point_raa(step, info, title, ax):
            reach_idx = info.get('reach_index')
            avoid_idx = info.get('avoid_index')
            ...
            reach_val = is_reach_np(np.array([info['x'][step].item(), info['y'][step].item()]))
        ...
        for step_n in indices: 
            # plt.figure(figsize=(12, 6*2))
            fig, axes = plt.subplots(2, 1, figsize=(4, 4), dpi=100)  # Smaller and square figure
            draw_point_raa(step_n, info, "Reach Avoid", axes[0])
            if config['EXP_NAME'] == 'PointReachAlwaysAvoid':
