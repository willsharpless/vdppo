...
from rraa_rl.env.general_task.deliveryrealv2_base import DeliveryRealv2Base, DeliveryRealv2BaseCfg, DeliveryRealv2BaseState
from rraa_rl.env.general_task.env import (EnvCfg, EnvUsingBase, StateWithTemporalNode, StaticTemporalNodeMixin,
                                              StaticTemporalNodeMixinCfg)


@define(slots=False)
class DeliveryRealv2Cfg(EnvCfg, StaticTemporalNodeMixinCfg):
    ...
    base: DeliveryRealv2BaseCfg = DeliveryRealv2BaseCfg()


class DeliveryRealv2(StaticTemporalNodeMixin, EnvUsingBase):
    """
    ...
    Cfg = DeliveryRealv2Cfg
    State = StateWithTemporalNode[DeliveryRealv2BaseState]

    def __init__(self, cfg: DeliveryRealv2Cfg = DeliveryRealv2Cfg()):
        self.cfg = cfg
        base_env = DeliveryRealv2Base(cfg.base, should_term_fn=self.should_terminate)
        EnvUsingBase.__init__(self, cfg, self.specification, base_env)
        StaticTemporalNodeMixin.__init__(self, cfg)
        self.base = base_env

    @staticmethod
    def create(cfg: DeliveryRealv2Cfg) -> "DeliveryRealv2":
        return DeliveryRealv2(cfg)
