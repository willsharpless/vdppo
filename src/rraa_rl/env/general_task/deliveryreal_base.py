...
class DeliveryRealBase(BaseEnv):
    ...
    def next_state(self, state: DeliveryRealBaseState, control: jnp.ndarray):
        ...
        return DeliveryRealBaseState(herd_state=herd_state_new, herder_state=herder_state_new, steps=state.steps + 1, centers=centers)

    ## BOOL PREDICATES (SPARSE)

    def is_herder_collide(self, state: DeliveryRealBaseState):
        herder_pos = state.herder_state[:, 0:2]
    ...
    def is_herd_herded(self, state: DeliveryRealBaseState):
    ...
    def is_herder_in_target(self, state: DeliveryRealBaseState, which=jnp, center=[0., 0.], radius=0.5):
    ...
    def get_predicates_bool(self, state: DeliveryRealBaseState):
        ...
        # if self.cfg.sdf_obstacles:
        #     predicates["obstacles"] = self.is_herder_in_obstacles_sdf(state)
    ...
    def sample_pos_outside_obst(self, key: PRNGKeyArray, herd_pos: jnp.ndarray, maxpos_per_ag: jnp.ndarray, minpos_per_ag: jnp.ndarray):
        ...
        def sample_valid_for_ag(key, ag_ix):
            ...
            def sample_valid_position(key):
                ...
                state_try = DeliveryRealBaseState(herd_state=herd_pos, herder_state=herder_state_try, steps=0, centers=centers)
