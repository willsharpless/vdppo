...
from rraa_rl.env.general_task.deliveryreal_base import DeliveryRealBase, DeliveryRealBaseCfg, DeliveryRealBaseState
...
@define(slots=False)
class DeliveryRealCfg(EnvCfg, StaticTemporalNodeMixinCfg):
    ...
    base: DeliveryRealBaseCfg = DeliveryRealBaseCfg()


class DeliveryReal(StaticTemporalNodeMixin, EnvUsingBase):
    """
    ...
    Cfg = DeliveryRealCfg
    State = StateWithTemporalNode[DeliveryRealBaseState]

    def __init__(self, cfg: DeliveryRealCfg = DeliveryRealCfg()):
        self.cfg = cfg
        base_env = DeliveryRealBase(cfg.base, should_term_fn=self.should_terminate)
        EnvUsingBase.__init__(self, cfg, self.specification, base_env)
        StaticTemporalNodeMixin.__init__(self, cfg)
        self.base = base_env

    @staticmethod
    def create(cfg: DeliveryRealCfg) -> "DeliveryReal":
        return DeliveryReal(cfg)
