...
from vdppo.env.general_task.delivery import Delivery, DeliveryBaseCfg
...
def animate_eval_trajs_single_agent(p: CallbackProps):
    ...
    figsize = np.array([4 * ncol, 3])
    fig, ax = plt.subplots(1, ncol, figsize=figsize, dpi=200, layout="none")
        ...
        for kk in pbar:
            ...
            for ii, ax in enumerate([ax]):
                ...
                for jj, traj in enumerate(trajs):
                    ...
                    facecolors.append(f"C{temporal_node_idx}")
            ...
            for ii in range(n_traj_anim):
                for jj in range(n_temporal_nodes):
                    ...
                    for agent_idx, circ in enumerate(circs):
                        ...
                        temporal_node_idx = T_state.temporal_node_idx[t_idx]
                        circ.set_facecolor(f"C{temporal_node_idx}")
...
def animate_eval_trajs_multi_agent_swarm(p: CallbackProps):
    plots_dir = p.run.plots_dir
    env: Delivery = p.env
    cfg = env.base.cfg
    close_radius = 0.3
