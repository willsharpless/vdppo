...
def compare_states(
    ...
    if np.any(qpos_diff > atol):
        ...
        logger.warning(
            f"[Step {step_count}] qpos mismatch: max_diff={np.max(qpos_diff):.6f} at idx={max_idx}, jax={jax_qpos[max_idx]:.6f}, np={np_qpos[max_idx]:.6f}"
        )
    ...
    if np.any(qvel_diff > atol):
        ...
        logger.warning(
            f"[Step {step_count}] qvel mismatch: max_diff={np.max(qvel_diff):.6f} at idx={max_idx}, jax={jax_qvel[max_idx]:.6f}, np={np_qvel[max_idx]:.6f}"
        )
    ...
    if np.any(ctrl_diff > atol):
        ...
        logger.warning(
            f"[Step {step_count}] ctrl mismatch: max_diff={np.max(ctrl_diff):.6f} at idx={max_idx}, jax={jax_ctrl[max_idx]:.6f}, np={np_ctrl[max_idx]:.6f}"
        )
    ...
    if jax_obs.shape != np_obs.shape:
        ...
        if np.any(obs_diff > atol):
            ...
            logger.warning(
                f"[Step {step_count}] obs mismatch in common prefix: max_diff={np.max(obs_diff):.6f} at idx={max_idx}"
            )
    else:
        ...
        if np.any(obs_diff > atol):
            ...
            logger.warning(
                f"[Step {step_count}] obs mismatch: max_diff={np.max(obs_diff):.6f} at idx={max_idx} ({obs_label}), jax={jax_obs[max_idx]:.6f}, np={np_obs[max_idx]:.6f}"
            )
            ...
            if len(mismatch_indices) <= 10:
                for idx in mismatch_indices:
                    ...
                    logger.info(
                        f"  obs[{idx}] ({label}): jax={jax_obs[idx]:.6f}, np={np_obs[idx]:.6f}, diff={obs_diff[idx]:.6f}"
                    )
    ...
    if np.any(effector_pos_diff > atol):
        ...
        logger.warning(
            f"[Step {step_count}] effector_pos mismatch: jax={jax_effector_pos}, np={np_effector_pos}, diff={effector_pos_diff}"
        )
...
def _print_obs_breakdown_jax(env_jax: SceneEnvJax, state_jax: SceneEnvState) -> None:
    ...
    for i in range(config.num_buttons):
        ...
        button_pos = np.asarray(
            jax.device_get(mjx_data.qpos[env_jax._button_joint_qpos_addrs[i] : env_jax._button_joint_qpos_addrs[i] + 1])
        )
        button_vel = np.asarray(
            jax.device_get(mjx_data.qvel[env_jax._button_joint_qpos_addrs[i] : env_jax._button_joint_qpos_addrs[i] + 1])
        )
        logger.info(
            f"  button_{i}_pos: shape={button_pos.shape}, using qpos_addr={env_jax._button_joint_qpos_addrs[i]}"
        )
        logger.info(
            f"  button_{i}_vel: shape={button_vel.shape}, using qpos_addr={env_jax._button_joint_qpos_addrs[i]} (SHOULD USE DOFADR!)"
        )
    ...
    drawer_pos = np.asarray(
        jax.device_get(mjx_data.qpos[env_jax._drawer_joint_qpos_addr : env_jax._drawer_joint_qpos_addr + 1])
    )
    drawer_vel = np.asarray(
        jax.device_get(mjx_data.qvel[env_jax._drawer_joint_qpos_addr : env_jax._drawer_joint_qpos_addr + 1])
    )
    logger.info(f"  drawer_pos: shape={drawer_pos.shape}, using qpos_addr={env_jax._drawer_joint_qpos_addr}")
    logger.info(
        f"  drawer_vel: shape={drawer_vel.shape}, using qpos_addr={env_jax._drawer_joint_qpos_addr} (SHOULD USE DOFADR!)"
    )
    obs_parts.append(("drawer_pos", drawer_pos.shape[0]))
    obs_parts.append(("drawer_vel", drawer_vel.shape[0]))

    # Window
    window_pos = np.asarray(
        jax.device_get(mjx_data.qpos[env_jax._window_joint_qpos_addr : env_jax._window_joint_qpos_addr + 1])
    )
    window_vel = np.asarray(
        jax.device_get(mjx_data.qvel[env_jax._window_joint_qpos_addr : env_jax._window_joint_qpos_addr + 1])
    )
    logger.info(f"  window_pos: shape={window_pos.shape}, using qpos_addr={env_jax._window_joint_qpos_addr}")
    logger.info(
        f"  window_vel: shape={window_vel.shape}, using qpos_addr={env_jax._window_joint_qpos_addr} (SHOULD USE DOFADR!)"
    )
...
def compare_action_processing(
    ...
    jax_gripper_opening = float(jax.device_get(jnp.clip(mjx_data.qpos[env_jax._gripper_opening_joint_id] / 0.8, 0, 1)))
    ...
    if gripper_diff > atol:
        ...
        logger.warning(
            f"[Step {step_count}] pre_gripper_opening mismatch: jax={jax_gripper_opening:.6f}, np={np_gripper_opening:.6f}"
        )
...
def main(which: str = "numpy"):
    ...
    record = True
    video_start_iter = 0  # Start recording after this many iterations
    video_max_frames = 200  # Maximum number of frames to record
    # video_max_frames = 3  # Maximum number of frames to record
    # video_max_frames = 50  # Maximum number of frames to record
    video_output_path = "play_manip_output_{}.mp4".format(which)
    video_fps = 30
    video_width = 640
    video_height = 480
    video_camera = 0  # -1 for free camera, or camera name/id
    video_frames = []
    ...
    logger.info("jax n_step: {} | np n_step: {}".format(env_jax.config.n_steps, env_np._n_steps))

    # Reset NumPy environment first (this will be the source of truth)
    logger.info("Resetting NumPy environment...")
    obs_np_initial, _ = env_np.reset(seed=1234)

    if TEST:
        # fmt: off
        qpos0 = np.array([-1.8283331e+00, -1.5327022e+00,  2.0594289e+00, -2.0733080e+00,
               -1.5707986e+00,  7.8851742e-01,  1.0008889e-01,  1.4333280e-04,
               9.9898197e-02, -9.5971443e-02,  1.0008893e-01,  1.4264567e-04,
               9.9901870e-02, -9.5977798e-02,  3.4786457e-01, -4.0477190e-02,
               1.9960716e-02,  7.2515893e-01, -3.9007024e-09,  3.5123544e-09,
               6.8858165e-01,  4.6151600e-04,  4.6151600e-04, -5.7473206e-03,
               5.2729961e-02])
        qvel0 = np.array([ 1.5663168e-04, -7.1288026e-03,  1.0672436e-01, -9.9147283e-02,
        6.3746115e-06,  5.7361793e-04,  1.7414317e-02,  1.6221427e-05,
        1.7436348e-02, -1.6605861e-02,  1.7413324e-02,  1.4669425e-05,
        1.7356258e-02, -1.6500792e-02,  1.9508502e-11,  5.7730432e-11,
        2.5665514e-07,  8.2687862e-10,  2.9379454e-09,  2.1118854e-18,
       -4.4135153e-09, -4.4135153e-09,  0.0000000e+00,  0.0000000e+00])
        # fmt: on
        env_np._data.qpos[:] = qpos0
        env_np._data.qvel[:] = qvel0
        mujoco.mj_forward(env_np._model, env_np._data)
    ...
    env_step_fn = jax.jit(env_step_fn, keep_unused=True)
    ...
    d = mujoco.MjData(m)
    d.qpos[:] = env_np._data.qpos
    d.qvel[:] = env_np._data.qvel

    # Get initial NumPy observation
    obs_np = env_np.compute_observation()

    # Capture initial end effector positions for comparison at exit
    initial_np_effector_pos = env_np._data.site_xpos[env_np._pinch_site_id].copy()
    initial_jax_effector_pos = np.asarray(jax.device_get(dx.site_xpos[env_jax._pinch_site_id]))
    logger.info(f"Initial NumPy effector pos: {initial_np_effector_pos}")
    logger.info(f"Initial JAX effector pos: {initial_jax_effector_pos}")
    ...
    with viewer:
        ...
        while True:
            ...
            # Video recording: start capturing frames after threshold
            if record and n_iters >= video_start_iter and len(video_frames) < video_max_frames:
                if not recording_started:
                    logger.info(f"Starting video recording at iteration {n_iters}")
                    recording_started = True
                # Render frame using offscreen renderer
                renderer.update_scene(d, camera=video_camera)
                frame = renderer.render()
                video_frames.append(frame)

                # Save video once we have enough frames
                if len(video_frames) >= video_max_frames:
                    logger.info(f"Saving video to {video_output_path} ({len(video_frames)} frames)")
                    imageio.mimwrite(video_output_path, video_frames, fps=video_fps)
                    logger.info(f"Video saved successfully!")

                    if which == "jax":
                        print("qpos")
                        print(repr(jax.device_get(dx.qpos)))
                        print("qvel")
                        print(repr(jax.device_get(dx.qvel)))

                    # Compare initial vs final end effector positions
                    print("\n" + "="*60)
                    print("END EFFECTOR POSITION COMPARISON")
                    print("="*60)
                    if which == "numpy":
                        final_effector_pos = env_np._data.site_xpos[env_np._pinch_site_id].copy()
                        delta = final_effector_pos - initial_np_effector_pos
                        print(f"NumPy Initial:  {initial_np_effector_pos}")
                        print(f"NumPy Final:    {final_effector_pos}")
                        print(f"NumPy Delta:    {delta}")
                        print(f"NumPy Delta XY: {delta[:2]} (should be ~0 for straight down motion)")
                        print(f"NumPy Delta Z:  {delta[2]:.6f}")
                    else:  # jax
                        final_effector_pos = np.asarray(jax.device_get(dx.site_xpos[env_jax._pinch_site_id]))
                        delta = final_effector_pos - initial_jax_effector_pos
                        print(f"JAX Initial:    {initial_jax_effector_pos}")
                        print(f"JAX Final:      {final_effector_pos}")
                        print(f"JAX Delta:      {delta}")
                        print(f"JAX Delta XY:   {delta[:2]} (should be ~0 for straight down motion)")
                        print(f"JAX Delta Z:    {delta[2]:.6f}")
                    print("="*60 + "\n")
            ...
            # # Set data from numpy env (env_np.data) to display env data (d).
            # d.qpos[:] = env_np._data.qpos
            # d.qvel[:] = env_np._data.qvel

            if True or _VIEWER_GLOBAL_STATE["running"]:
                print(f"\n=== Step {n_iters + 1} ===")
                ...
                if which == "jax":
                    # set NumPy env data to jax env data.
                    env_np._data.qpos[:] = jax.device_get(dx.qpos)
                    env_np._data.qvel[:] = jax.device_get(dx.qvel)
                    mujoco.mj_forward(env_np._model, env_np._data)

                    obs_np, reward_np, terminated_np, truncated_np, info_np = env_np.step(action_np_normalized)
                    qpos_target, target_gripper = info_np["qpos_target"], info_np["target_gripper"]

                    logger.debug("Stepping JAX environment...")
                    # out_jax: ManipStep = env_step_fn(state_jax, action_normalized)
                    out_jax: ManipStep = env_step_fn(state_jax, qpos_target, target_gripper)
