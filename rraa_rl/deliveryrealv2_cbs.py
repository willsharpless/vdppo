...
def plot_eval_trajs(p: CallbackProps):
    ...
    bT_states: list[DeliveryRealv2.State] = [traj.state_now for traj in p.bT_test_rollouts]
    ...
    for ii, ax in enumerate(axes):
        ...
        for traj in p.bT_test_rollouts[start_idx:end_idx]:
            ...
            T_state: DeliveryRealv2.State = traj.state_now
...
def animate_eval_trajs_multi_agent(p: CallbackProps):
    ...
    bT_states: list[DeliveryRealv2.State] = [traj.state_now for traj in bT_test_rollouts]
    b_temporal_idx = np.array([T_state.temporal_node_idx[0] for T_state in bT_states])
    ...
    if cfg.dynamic_targets:
        plot_predicates = ["oob", "obstacles", "air_obstacles"]
...
class PlotRootTrajPreds(struct.PyTreeNode):
    ...
    def __call__(self, p: CallbackProps):
        ...
        for bb in range(n_plot):
            ...
            T_state: DeliveryRealv2.State = trajs[bb].state_now
...
def viz_collect_data(p: CallbackProps):
    ...
    Tb_state: DeliveryRealv2.State = Tb_rollout.state_now
    bT_A, bT_Q, bT_temporal_idx = agent.compute_A_Q(Tb_rollout, debug=True)

    b_Q = b_data.Q
    n_temporal_nodes = env.n_temporal_nodes
    # ---------------------------------------------------------------
    b_state: DeliveryRealv2.State = b_data.state
...
def env_layout_plot(p: CallbackProps):
    ...
    if p.train_step == 0:
        ...
        if cfg.dynamic_targets:
            plot_predicates = ["obstacles"] if not cfg.air_obstacles else ["obstacles", "air_obstacles"]
        ...
        for pred_name in plot_predicates:
            if pred_name in bb_predicates:
                ...
                ax.contourf(
                    bb_X, bb_Y, bb_predicates[pred_name], levels=[0, 1], colors=color, alpha=alpha
                )
        ...
        for agent_idx, circ in enumerate(agent_circs):
            pos = [[-2.1, 0.3], [3., -0.2], [0.1, -0.1]][agent_idx]  # fixed positions for layout
            circ.center = pos
        ...
        if cfg.dynamic_targets:
            for target_idx in range(2):
                ...
                circ.center = [[-3.8, 1.9], [1.9, -4.1]][target_idx]  # fixed positions for layout
                ax.add_patch(circ)
...
def air_obstacles_to_aabbs(cfg, which=jnp, agent_radius=0.2):
    """
    Returns AABBs for all obstacles.

    Output shape: (n_obst, 4) with columns [xmin, ymin, xmax, ymax]
    """
    centers = which.array(cfg.air_obstacle_centers)  # (n_obst, 2)
    radii = which.array(cfg.air_obstacle_radiuses)  # (n_obst,)
    ratios = which.array(cfg.air_obstacle_lw_ratios)  # (n_obst,)
...
def animate_deliveryrealv2_traj(
    ...
    air_aabbs = air_obstacles_to_aabbs(cfg, which=np)
