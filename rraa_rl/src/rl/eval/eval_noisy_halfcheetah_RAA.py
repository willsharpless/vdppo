...
if __name__ == "__main__":
    ...
    experiment_dirs = [
        "NOISY_halfcheetah_raa_rg_noisy",
        "NOISY_halfcheetah_raa_sparse_noisy",
        "NOISY_halfcheetah_raa_morl_noisy",
        "NOISY_halfcheetah_raa_cppo_kp100_noisy",
    ]
    log_file_dir = "model"
    ...
    color_dict = {
        "NOISY_halfcheetah_raa_rg_noisy": colors["DOHJPPO"], 
    ...
    for ax, noise_tag in zip(axes, noise_tags):
        for exp_dir in experiment_dirs:
            ...
            ax.plot(np.array(scores[:, 0], dtype=int), scores[:, data_to_plot_ix], 
                    label=exp_dir, color=color_dict[exp_dir], linewidth=1)

        ax.set_title(r"{}".format(noise_tag))
        ax.set_xlabel(r"Epoch")
        if data_to_plot_ix == 1:
            ax.set_ylim([-0.2, 0.])
        elif data_to_plot_ix == -1:
            ax.set_ylim([0., 1.])
