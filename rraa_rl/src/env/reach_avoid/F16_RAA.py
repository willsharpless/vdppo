...
class F16ReachAvoid(environment.Environment):
    ...
    def step_env(
        ...
        observation = jnp.concatenate([self.get_obs(next_state_new), jnp.array([avoid_value, reach_value])]).squeeze()
    ...
    def reset_env(
        ...
        time = 0
        env_state = EnvStateRA(state, time, avoid_value, reach_value, has_reached)
...
class F16AvoidOnly(environment.Environment):
    ...
    def step_env(
        self,
        key: chex.PRNGKey,
        state: EnvStateAvoidOnly,
        action: chex.Array,
        params: EnvParams,
    ) -> Tuple[chex.Array, EnvStateAvoidOnly, float, bool, dict]:
    ...
    def reset_env(
        ...
        time = 0
        env_state = EnvStateAvoidOnly(state, time, avoid_value)
        observation = jnp.concatenate([self.get_obs(env_state), jnp.array([avoid_value, reach_value])]) # might need a squeeze here
    ...
    def get_obs(self, state: EnvStateAvoidOnly) -> chex.Array:
        """Return angle in polar coordinates and change."""
    ...
    def is_terminal(self, state: EnvStateAvoidOnly, params: EnvParams) -> bool:
        """Check whether state is terminal."""
        # Check number of steps in episode termination condition
        done = state.time >= params.max_steps_in_episode
        return done
