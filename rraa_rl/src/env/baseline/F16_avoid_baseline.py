...
class F16AvoidBaseline(environment.Environment):
    ...
    def step_env(
        ...
        next_energy = jnp.clip(state.energy - jnp.sum(control_raw ** 2) / 3, params.min_energy, params.max_energy)
    ...
    def is_reach(self, state, avoid_value, params: EnvParams) -> float:
        ...
        # has_reached_goal = jnp.fabs(state[self.H] - 375.) < 25.
        # reach = (jnp.fabs(state[self.H] - 375.) - 25.) / 2
    ...
    def is_avoid(self, state, params: EnvParams):
        # angle_eps = 0.1
        # in_morelli_0 = ((state[self.ALPHA] - angle_eps < self.MORELLI_BOUNDS[self.ALPHA, 1]) &
        #                 (state[self.ALPHA] + angle_eps > self.MORELLI_BOUNDS[self.ALPHA, 0]))
        # in_morelli_1 = ((state[self.BETA] - angle_eps < self.MORELLI_BOUNDS[self.BETA, 1]) &
        #                 (state[self.BETA] + angle_eps > self.MORELLI_BOUNDS[self.BETA, 0]))
        # pitch_valid = jnp.abs(state[self.THETA]) < np.pi / 2
        ...
        # avoid_1 = (-200.0 <= state[self.PE]) & (state[self.PE] <= -0.0) & (jnp.fabs(state[self.PN] - 500.) <= 25.)
        # avoid_2 = (0.0 <= state[self.PE]) & (state[self.PE] <= 200.0) & (jnp.fabs(state[self.PN] - 1000.) <= 25.)
        # avoid_3 = (-200.0 <= state[self.PE]) & (state[self.PE] <= -0.0) & (jnp.fabs(state[self.PN] - 1500.) <= 25.)

        return jnp.logical_not(alt_valid & pe_valid)

        # return jnp.logical_not(alt_valid & pe_valid) | avoid_1 | avoid_2 | avoid_3
