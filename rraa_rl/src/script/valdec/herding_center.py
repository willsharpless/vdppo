...
MAX_ACCELERATION = 1.  # maximum acceleration for any agent
# REL_ACEL = [0.01, 0.05, 0.01, 0.01, 0.01]  # relative acceleration limits for each agent
# MAX_ACCELERATION = 0.05  # maximum acceleration for any agent
...
config["NUM_EPISODE"]=3000 # DEBUG FIXME not used other than in long eval

if False: # DEBUG FIXME
    config["NUM_ENVS"]=128
...
def main():
    ...
    # High accel var for 
    i = 0 # slow agent
    obs_std = obs_std.at[i * env.obs_size_per_agent + 4].set(100.)  # ax
    obs_std = obs_std.at[i * env.obs_size_per_agent + 5].set(100.)  # ay
    i = 1 # fast agent
    obs_std = obs_std.at[i * env.obs_size_per_agent + 4].set(1000.)  # ax
    obs_std = obs_std.at[i * env.obs_size_per_agent + 5].set(1000.)  # ay
    for i in [2, 3, 4]: # evaders
        obs_std = obs_std.at[i * env.obs_size_per_agent + 4].set(50.)  # ax
        obs_std = obs_std.at[i * env.obs_size_per_agent + 5].set(50.)  # ay
    ...
    # i = 0 # slow agent
    # obs_std = obs_std.at[i * env.obs_size_per_agent + 4].set(5.)  # ax
    # obs_std = obs_std.at[i * env.obs_size_per_agent + 5].set(5.)  # ay
    # i = 1 # fast agent
    # obs_std = obs_std.at[i * env.obs_size_per_agent + 4].set(25.)  # ax
    # obs_std = obs_std.at[i * env.obs_size_per_agent + 5].set(25.)  # ay
    # for i in [2, 3, 4]: # evaders
    #     obs_std = obs_std.at[i * env.obs_size_per_agent + 4].set(5.)  # ax
    #     obs_std = obs_std.at[i * env.obs_size_per_agent + 5].set(5.)  # ay
