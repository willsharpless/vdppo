...
def main():
    ...
    class DummyRALoopDAG: # DEBUG FIXME placeholder, fake node numbers
        def __init__(self, just_RAA=False):
            ...
            self.predicates = ["reach3_any", "obstacles"]
    ...
    # FIXME, update for dynamic predicates now?
    # for i, name in enumerate(env.dynamic_predicate_names):
    #     obs_mean = obs_mean.at[env._env.observation_size + i].set(env.default_tracked_locs[i, 0])
    #     obs_std = obs_std.at[env._env.observation_size + i].set(10000)
    obs_mean = obs_mean.at[env._env.observation_size + 2].set(reach3_stats["mean_x"])
    obs_mean = obs_mean.at[env._env.observation_size + 3].set(reach3_stats["mean_y"])
    obs_std = obs_std.at[env._env.observation_size + 2].set(reach3_stats["std_x"])
    obs_std = obs_std.at[env._env.observation_size + 3].set(reach3_stats["std_y"])
...
def _get_contour_data(target_loc=None):
    """Precompute contour data for reach and avoid predicates.
    
    Args:
        target_loc: Optional target location [x, y] for dynamic reach predicates.
    ...
    model = MultiPointDynamicGeneralTask(
        active_predicates=["reach1_any", "reach2_any", "reach3_any", "obstacles"], 
        negated_predicate_mask=jnp.array([1, 1, 1, 0]),
        n_agents=config["N_AGENTS"],
        fixed_velocity=config["FIXED_VELOCITY"],
        dynamic_predicate_names=[],  # Don't track for contour evaluation
    )
    
    # Use provided target or default
    if target_loc is None:
        target_loc = jnp.array([0.25, 0.7])
    else:
        target_loc = jnp.array(target_loc)
    ...
    def eval_predicates(obs):
        ...
        reach1_val, _ = model.is_reach1_any(dummy_state, target_loc)
        reach2_val, _ = model.is_reach2_any(dummy_state, target_loc)
        reach3_val, _ = model.is_reach3_any(dummy_state, target_loc)
        avoid_val, _ = model.is_obstacles(dummy_state, target_loc)
...
def _plot_contours(ax, X, Y, reach1_values, reach2_values, reach3_values, avoid_values, mode="rraa", plot_zero_level_only=False):
    """Plot contours based on mode.
    
    Args:
        plot_zero_level_only: If True, only plot zero level sets as lines (for dynamic visualization).
                             If False, plot filled contours (default behavior).
    """
    if plot_zero_level_only:
        # Plot only zero level sets as line contours
        if mode == "rraa":
            ax.contour(X, Y, reach1_values, levels=[0], colors=['green'], linewidths=2, alpha=0.8)
            ax.contour(X, Y, reach2_values, levels=[0], colors=['blue'], linewidths=2, alpha=0.8)
        elif mode == "raa1":
            ax.contour(X, Y, reach1_values, levels=[0], colors=['green'], linewidths=2, alpha=0.8)
        elif mode == "raa2":
            ax.contour(X, Y, reach2_values, levels=[0], colors=['blue'], linewidths=2, alpha=0.8)
        elif mode == "raa3":
            ax.contour(X, Y, reach3_values, levels=[0], colors=['purple'], linewidths=2, alpha=0.8)
        # Always plot obstacle zero level set
        ax.contour(X, Y, avoid_values, levels=[0], colors=['red'], linewidths=2, alpha=0.8)
    else:
        # Original filled contour plotting
        if mode == "rraa":
...
def _draw_agents(ax, info, step_idx, alpha, reach1_values, reach2_values, reach3_values, avoid_values, mode="rraa", predicate_funcs=None, dynamic_target_loc=None):
    ...
    Args:
        ...
        dynamic_target_loc: Optional target location [x, y] for dynamic reach predicates.
    """
    ...
    # Use dynamic target location if provided, otherwise use default
    if dynamic_target_loc is None:
        dynamic_target_loc = jnp.array([0.25, 0.7])
    ...
    for agent_idx, (x_key, y_key) in enumerate(agent_keys):
        ...
        reach1_val = predicate_funcs['is_reach1'](dummy_state, dynamic_target_loc)[0]
        reach2_val = predicate_funcs['is_reach2'](dummy_state, dynamic_target_loc)[0]
        reach3_val = predicate_funcs['is_reach3'](dummy_state, dynamic_target_loc)[0]
        avoid_val = predicate_funcs['is_obstacles'](dummy_state, dynamic_target_loc)[0]
...
def plot_contour_multipoint(multi_info, epoch, config, policy_decision_sample=None, just_RAA=False):
    ...
    # Check if we have dynamic target tracking
    has_dynamic_target = 'tracked_loc_reach3_any_x' in info_raa and 'tracked_loc_reach3_any_y' in info_raa
    
    # For static plots with dynamic targets, we'll overlay multiple timesteps
    if has_dynamic_target:
        ...
        # Sample multiple timesteps to show target evolution
        num_target_samples = 11
        target_sample_indices = np.linspace(0, full_len - 1, num_target_samples, dtype=int)

    # Precompute static contours (obstacles) once
    static_target_loc = jnp.array([0.25, 0.7])
    X, Y, reach1_values, reach2_values, reach3_values, avoid_values = _get_contour_data(static_target_loc)
    ...
    def draw_point_rraa(info, title, ax, mode="a"):
        ...
        # Plot contours as zero level sets only (line contours)
        _plot_contours(ax, X, Y, reach1_values, reach2_values, reach3_values, avoid_values, mode, plot_zero_level_only=True)
        
        # If we have dynamic targets, overlay multiple timesteps of target locations
        if has_dynamic_target and mode == "raa3":
            for i, sample_idx in enumerate(target_sample_indices):
                target_x = info['tracked_loc_reach3_any_x'][sample_idx].item()
                target_y = info['tracked_loc_reach3_any_y'][sample_idx].item()
                
                # Recompute contours for this target location
                _, _, _, _, reach3_vals_t, _ = _get_contour_data(jnp.array([target_x, target_y]))
                
                # Plot zero level set with varying alpha (fading from early to late)
                alpha = 0.3 + 0.5 * (i / (num_target_samples - 1))
                ax.contour(X, Y, reach3_vals_t, levels=[0], colors=['purple'], 
                          linewidths=1.5, alpha=alpha, linestyles='--')
                
                # Mark target center
                marker_alpha = 0.4 + 0.4 * (i / (num_target_samples - 1))
                ax.plot(target_x, target_y, 'x', color='purple', 
                       markersize=8, markeredgewidth=2, alpha=marker_alpha)
        
        # Draw trajectory
        indices = np.linspace(0, full_len - 1, 11, dtype=int)
        for step_n, i in enumerate(indices):
            alpha = (step_n + 1) / 11
            # Extract dynamic target at this timestep if available
            step_target_loc = static_target_loc
            if has_dynamic_target:
                target_x = info['tracked_loc_reach3_any_x'][i].item()
                target_y = info['tracked_loc_reach3_any_y'][i].item()
                step_target_loc = jnp.array([target_x, target_y])
            _draw_agents(ax, info, i, alpha, reach1_values, reach2_values, reach3_values, avoid_values, mode, dynamic_target_loc=step_target_loc)
        
        # Highlight special events
        if reach1_idx > -1 and mode in ["rraa", "raa1"]:
            step_target_loc = static_target_loc
            if has_dynamic_target:
                target_x = info['tracked_loc_reach3_any_x'][reach1_idx].item()
                target_y = info['tracked_loc_reach3_any_y'][reach1_idx].item()
                step_target_loc = jnp.array([target_x, target_y])
            _draw_agents(ax, info, reach1_idx, 0.5, reach1_values, reach2_values, reach3_values, avoid_values, mode, dynamic_target_loc=step_target_loc)
        if reach2_idx > -1 and mode in ["rraa", "raa2"]:
            step_target_loc = static_target_loc
            if has_dynamic_target:
                target_x = info['tracked_loc_reach3_any_x'][reach2_idx].item()
                target_y = info['tracked_loc_reach3_any_y'][reach2_idx].item()
                step_target_loc = jnp.array([target_x, target_y])
            _draw_agents(ax, info, reach2_idx, 0.5, reach1_values, reach2_values, reach3_values, avoid_values, mode, dynamic_target_loc=step_target_loc)
        if reach3_idx > -1 and mode in ["rraa", "raa3"]:
            step_target_loc = static_target_loc
            if has_dynamic_target:
                target_x = info['tracked_loc_reach3_any_x'][reach3_idx].item()
                target_y = info['tracked_loc_reach3_any_y'][reach3_idx].item()
                step_target_loc = jnp.array([target_x, target_y])
            _draw_agents(ax, info, reach3_idx, 0.5, reach1_values, reach2_values, reach3_values, avoid_values, mode, dynamic_target_loc=step_target_loc)
        if crash_idx > -1:
            step_target_loc = static_target_loc
            if has_dynamic_target:
                target_x = info['tracked_loc_reach3_any_x'][crash_idx].item()
                target_y = info['tracked_loc_reach3_any_y'][crash_idx].item()
                step_target_loc = jnp.array([target_x, target_y])
            _draw_agents(ax, info, crash_idx, 0.5, reach1_values, reach2_values, reach3_values, avoid_values, mode, dynamic_target_loc=step_target_loc)
...
def plot_video_contour_raa_loop(multi_info, epoch, config, save_video=False, prefix="", log_wandb=True):
    ...
    # Extract dynamic target location from first frame for contour computation
    dynamic_target_loc = None
    if 'tracked_loc_reach3_any_x' in info_raa and 'tracked_loc_reach3_any_y' in info_raa:
        target_x = info_raa['tracked_loc_reach3_any_x'][0].item()
        target_y = info_raa['tracked_loc_reach3_any_y'][0].item()
        dynamic_target_loc = jnp.array([target_x, target_y])
    
    # Precompute contour data once with initial target location
    X, Y, reach1_values, reach2_values, reach3_values, avoid_values = _get_contour_data(dynamic_target_loc)
    ...
    def draw_point_rraa(step, info, title, ax, mode="rraa"):
        ...
        # Extract dynamic target at this timestep
        step_target_loc = dynamic_target_loc
        if 'tracked_loc_reach3_any_x' in info and 'tracked_loc_reach3_any_y' in info:
            target_x = info['tracked_loc_reach3_any_x'][step].item()
            target_y = info['tracked_loc_reach3_any_y'][step].item()
            step_target_loc = jnp.array([target_x, target_y])
        
        # Plot contours
        _plot_contours(ax, X, Y, reach1_values, reach2_values, reach3_values, avoid_values, mode)
        
        # Draw current agents (reusing cached functions) with current target location
        cached_predicate_funcs = _draw_agents(ax, info, step, 0.9, reach1_values, reach2_values, reach3_values, avoid_values, mode, cached_predicate_funcs, step_target_loc)
        
        # Highlight special events if they've occurred
        if reach1_idx > -1 and step >= reach1_idx and mode in ["rraa", "raa1"]:
            event_target_loc = step_target_loc
            if 'tracked_loc_reach3_any_x' in info and 'tracked_loc_reach3_any_y' in info:
                target_x = info['tracked_loc_reach3_any_x'][reach1_idx].item()
                target_y = info['tracked_loc_reach3_any_y'][reach1_idx].item()
                event_target_loc = jnp.array([target_x, target_y])
            cached_predicate_funcs = _draw_agents(ax, info, reach1_idx, 0.5, reach1_values, reach2_values, reach3_values, avoid_values, mode, cached_predicate_funcs, event_target_loc)
        if reach2_idx > -1 and step >= reach2_idx and mode in ["rraa", "raa2"]:
            ...
            if 'tracked_loc_reach3_any_x' in info and 'tracked_loc_reach3_any_y' in info:
                ...
                target_y = info['tracked_loc_reach3_any_y'][reach2_idx].item()
                event_target_loc = jnp.array([target_x, target_y])
            cached_predicate_funcs = _draw_agents(ax, info, reach2_idx, 0.5, reach1_values, reach2_values, reach3_values, avoid_values, mode, cached_predicate_funcs, event_target_loc)
        if reach3_idx > -1 and step >= reach3_idx and mode in ["rraa", "raa3"]:
            ...
            if 'tracked_loc_reach3_any_x' in info and 'tracked_loc_reach3_any_y' in info:
                ...
                target_y = info['tracked_loc_reach3_any_y'][reach3_idx].item()
                event_target_loc = jnp.array([target_x, target_y])
            cached_predicate_funcs = _draw_agents(ax, info, reach3_idx, 0.5, reach1_values, reach2_values, reach3_values, avoid_values, mode, cached_predicate_funcs, event_target_loc)
        if crash_idx > -1 and step >= crash_idx:
            ...
            if 'tracked_loc_reach3_any_x' in info and 'tracked_loc_reach3_any_y' in info:
                ...
                target_y = info['tracked_loc_reach3_any_y'][crash_idx].item()
                event_target_loc = jnp.array([target_x, target_y])
            cached_predicate_funcs = _draw_agents(ax, info, crash_idx, 0.5, reach1_values, reach2_values, reach3_values, avoid_values, mode, cached_predicate_funcs, event_target_loc)
        
        # Draw dynamic target location if it exists
        if step_target_loc is not None and mode != 'a':
            # Draw target marker
            ax.plot(step_target_loc[0], step_target_loc[1], 'x', color='purple', 
                   markersize=15, markeredgewidth=3, label='Dynamic Target')
            # Draw target circle
            target_circle = plt.Circle((step_target_loc[0], step_target_loc[1]), 
                                      0.3, color='purple', fill=False, linewidth=2, linestyle='--')
