...
class F16ReachReach(environment.Environment):
    ...
    def step_env(
        ...
        observation = jnp.concatenate([self.get_obs(next_state_new), jnp.array([reach1_value, reach2_value])]).squeeze()
    ...
    def reset_env(
        ...
        time = 0
        env_state = EnvStateRR(state, time, reach1_value, reach2_value, has_reached_1, has_reached_2)
...
class F16Reach1(environment.Environment):
    ...
    def step_env(
        ...
        action: chex.Array,
        params: EnvParams,
    ) -> Tuple[chex.Array, EnvStateR1, float, bool, dict]:
        ...
        # next_state_new = EnvStateR1(state.state, state.time + 1, reach1_value) # IDK which is right
        next_state_new = EnvStateR1(a_state_new, state.time + 1, reach1_value)
    ...
    def is_terminal(self, state: EnvStateR1, params: EnvParams) -> bool:
        """Check whether state is terminal."""
        # Check number of steps in episode termination condition
        done = state.time >= params.max_steps_in_episode
        return done
...
class F16Reach2(environment.Environment):
    ...
    def step_env(
        ...
        action: chex.Array,
        params: EnvParams,
    ) -> Tuple[chex.Array, EnvStateR2, float, bool, dict]:
        ...
        # next_state_new = EnvStateR2(state.state, state.time + 1, reach2_value) # IDK which is right
        next_state_new = EnvStateR2(a_state_new, state.time + 1, reach2_value)
    ...
    def reset_env(
        ...
        time = 0
        env_state = EnvStateR2(state, time, reach2_value)
        observation = jnp.concatenate([self.get_obs(env_state), jnp.array([reach1_value, reach2_value])]) # might need a squeeze here
    ...
    def is_terminal(self, state: EnvStateR2, params: EnvParams) -> bool:
        """Check whether state is terminal."""
        # Check number of steps in episode termination condition
        done = state.time >= params.max_steps_in_episode
        return done
...
class F16ReachReachBaseline(environment.Environment):
    ...
    def step_env(
        self,
        key: chex.PRNGKey,
        state: EnvStateRRDecomposed,
        action: chex.Array,
        params: EnvParams,
    ) -> Tuple[chex.Array, EnvStateRRDecomposed, float, bool, dict]:
        ...
        # next_state_new = EnvStateRR(state.state, state.time + 1, reach1_value, reach2_value, has_reached_1, has_reached_2)
        next_state_new = EnvStateRRDecomposed(a_state_new, state.time + 1, reach1_value, reach2_value, has_reached_1, has_reached_2)

        # observation = jnp.concatenate([self.get_obs(state), jnp.array([reach1_value, reach2_value])]) # might need a squeeze here
        observation = self.get_obs(next_state_new)
    ...
    def reset_env(
        self, key: chex.PRNGKey, params: EnvParams
    ) -> Tuple[chex.Array, EnvStateRRDecomposed]:
        ...
        time = 0
        env_state = EnvStateRRDecomposed(state, time, reach1_value, reach2_value, has_reached_1, has_reached_2)
        observation = self.get_obs(env_state)
    ...
    def get_obs(self, state: EnvStateRRDecomposed) -> chex.Array:
        """Return angle in polar coordinates and change."""
    ...
    def is_terminal(self, state: EnvStateRRDecomposed, params: EnvParams) -> bool:
        """Check whether state is terminal."""
        # Check number of steps in episode termination condition
        done = state.time >= params.max_steps_in_episode
        return done
    ...
    def observation_space(self, params: EnvParams) -> spaces.Box:
        """Observation space of the environment."""
        high = jnp.ones(24) * jnp.finfo(jnp.float32).max
        return spaces.Box(-high, high, shape=(24,), dtype=jnp.float32)
...
class F16Reach1Baseline(environment.Environment):
    ...
    def step_env(
        ...
        action: chex.Array,
        params: EnvParams,
    ) -> Tuple[chex.Array, EnvStateR, float, bool, dict]:
        ...
        next_state_new = EnvStateR(a_state_new, state.time + 1, reach1_value)
    ...
    def reset_env(
        ...
        time = 0
        env_state = EnvStateR(state, time, reach1_value)
        observation = self.get_obs(env_state)
        # observation = jnp.concatenate([self.get_obs(env_state), jnp.array([reach1_value, reach2_value])]) # might need a squeeze here
    ...
    def get_obs(self, state: EnvStateR) -> chex.Array:
        """Return angle in polar coordinates and change."""
    ...
    def is_terminal(self, state: EnvStateR, params: EnvParams) -> bool:
        """Check whether state is terminal."""
        # Check number of steps in episode termination condition
        done = state.time >= params.max_steps_in_episode
        return done
...
class F16Reach2Baseline(environment.Environment):
    ...
    def step_env(
        ...
        next_state_new = EnvStateR(a_state_new, state.time + 1, reach2_value)
    ...
    def reset_env(
        ...
        time = 0
        env_state = EnvStateR(state, time, reach2_value)
        observation = self.get_obs(env_state)
