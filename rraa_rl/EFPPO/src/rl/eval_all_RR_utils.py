...
def plot_scores_RR(traj_batches, config, title="HOPPER-RR", sub_plots_adjust=True, wspace=0.3, hspace=0.6, tight_layout=True, pad=0.1, w_pad=0.1, h_pad=0.1, use_stochastic=False):
    ...
    order = 1
    for i, label in enumerate(labels):
        axes[order].barh(label, -min_val_means[i], xerr=min_val_stds[i]/2, color=colors[label], capsize=4, zorder=1)
        axes[order].barh(label, -min_val_means[i], color=colors[label], capsize=4, zorder=2)
    axes[order].set_title(r"{}: Maximum Value ($\rightarrow$)".format(title), fontsize=12)
    axes[order].set_xlabel(r"Value")
    axes[order].set_yticks(np.arange(len(labels)))
    axes[order].set_yticklabels(labels, ha='right', fontsize=10)
    ...
    plt.savefig(f"model/{config['TEST_DIR']}/{config['NAME_TAG']}/suc_val_plot", dpi=300, bbox_inches="tight", pad_inches=0.1)
    ...
    plt.savefig(f"model/{config['TEST_DIR']}/{config['NAME_TAG']}/metric_plot", dpi=300, bbox_inches="tight", pad_inches=0.1)
    return fig
