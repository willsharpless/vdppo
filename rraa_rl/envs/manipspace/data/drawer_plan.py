...
from rraa_rl.envs.manipspace.data.plan_oracle import PlanOracle
...
class DrawerPlanOracle(PlanOracle):
    ...
    def reset(self, ob, info):
        ...
        print("initial: ", info['privileged/drawer_handle_pos'])
        print("goal: ", info['privileged/target_drawer_handle_pos'])
        print("--------------")

        times, poses, grasps = self.compute_keyframes(plan_input)
        labels = [name for name in times.keys()]
        ...
        self._t_max = times[-1]
        self._done = False
        self._plan, self._labels = self.compute_plan(times, labels, poses, grasps)
