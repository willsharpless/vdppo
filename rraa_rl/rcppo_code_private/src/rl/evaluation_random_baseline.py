...
from EFPPO.src.model.actorcritic import ActorCritic_Continuous
from EFPPO.src.rl.EFPPO_utils import _env_step_ppo
...
def test(env, env_params, config, rng):
    ...
    raw_restored = checkpoints.restore_checkpoint(
        ckpt_dir='/home/mepear_gc/EFPPO/model/{}/{}'.format(config["DIR"], config["DIR_MODEL"]),
        target=None
        )
