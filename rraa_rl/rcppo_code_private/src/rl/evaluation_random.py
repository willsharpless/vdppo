...
from EFPPO.src.model.actorcritic import Policy_Network, Value_Network, ActorCritic_Continuous
from EFPPO.src.rl.EFPPO_utils import _env_step, _env_step_deterministic, _env_step_test
from EFPPO.src.rl.root_finding import Bisection
from EFPPO.src.rl.utils import tree_index1, tree_index2
...
def test(env, env_params, config, rng):
    ...
    raw_restored = checkpoints.restore_checkpoint(ckpt_dir='/home/mepear_gc/EFPPO/model/{}/{}'.format(
        config["DIR"], config["DIR_MODEL"]), target=None)

    raw_restored_baseline_ppo = checkpoints.restore_checkpoint(ckpt_dir='/home/mepear_gc/EFPPO/model/{}_ppo/{}'.format(
        config["DIR"], config["DIR_MODEL"]), target=None)

    raw_restored_baseline_cppo = checkpoints.restore_checkpoint(ckpt_dir='/home/mepear_gc/EFPPO/model/{}_cppo/{}'.format(
        config["DIR"], config["DIR_MODEL"]), target=None)
